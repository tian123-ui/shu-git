...
def generate_hive_ddl(table, columns, output_dir="ddl_scripts"):
    """
    生成Hive DDL并保存到单独的文件
    ...
    hive_ddl +=f"LOCATION 'hdfs://cdh01:8020/bigdata_warehouse/tms/ods/ods_{table}'\n"
    hive_ddl += """
    TBLPROPERTIES (
    'orc.compress' = 'SNAPPY',
...
def generate_seatunnel_config(mysql_host, mysql_port, mysql_user, mysql_password,
    ...
    if has_create_time:
        print(f"  → 表包含create_time字段，添加增量同步逻辑")
        query = f"SELECT {columns_list},date_format(create_time,'%Y%m%d') as ds FROM `{database}`.`{table}`"
    else:
...
if __name__ == "__main__":
    ...
    for table in tables:
        ...
        # 生成Hive DDL文件（单独输出）
        generate_hive_ddl(table, columns, args.ddl_dir)
