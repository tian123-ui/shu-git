...
--TODO: 1. dws_trade_user_sku_order_1d
...
--TODO: 2. dws_trade_user_order_1d
...
--TODO: 3. dws_trade_user_cart_add_1d
...
--TODO: 4. dws_trade_user_payment_1d
...
--TODO: 5. dws_trade_province_order_1d
...
--TODO: 6. dws_tool_user_coupon_coupon_used_1d
...
--TODO: 7. dws_interaction_sku_favor_add_1d
...
--TODO: 8. dws_traffic_session_page_view_1d
...
insert overwrite table dws_traffic_session_page_view_1d partition(dt='20250728')
select
    session_id,
    mid_id,
    brand,
...
insert overwrite table dws_traffic_session_page_view_1d partition(dt='2025-07-28')
select
    session_id,
    mid_id,
    brand,
...
--TODO: 9. dws_traffic_page_visitor_page_view_1d
...
insert overwrite table dws_traffic_page_visitor_page_view_1d partition(dt='20250728')
select
    mid_id,
    brand,
    model,
...
--TODO: 2.1. dws_trade_user_sku_order_nd
...
insert overwrite table dws_trade_user_sku_order_nd partition(dt='2025-07-28')
select
    -- 维度字段：用户、商品、品类、品牌
    user_id,
    sku_id,
    ...
    tm_id,
    tm_name,
    -- 最近7日指标：通过if筛选近7天数据后求和
    sum(if(dt >= date_add('2025-07-28', -6), order_count_1d, 0)) as order_count_7d,
    sum(if(dt >= date_add('2025-07-28', -6), order_num_1d, 0)) as order_num_7d,
    sum(if(dt >= date_add('2025-07-28', -6), order_original_amount_1d, 0)) as order_original_amount_7d,
    sum(if(dt >= date_add('2025-07-28', -6), activity_reduce_amount_1d, 0)) as activity_reduce_amount_7d,
    sum(if(dt >= date_add('2025-07-28', -6), coupon_reduce_amount_1d, 0)) as coupon_reduce_amount_7d,
    sum(if(dt >= date_add('2025-07-28', -6), order_total_amount_1d, 0)) as order_total_amount_7d,
    -- 最近30日指标：直接求和近30天数据（where已筛选范围）
...
-- 筛选近30天数据（含7天），避免全表扫描
where dt >= date_add('2025-07-28', -29)
-- 按维度分组聚合（你提供的片段即此处末尾）
...
--TODO: 2.2. dws_trade_province_order_nd
...
insert overwrite table dws_trade_province_order_nd partition(dt='2025-07-28')
select
    province_id,
    province_name,
    area_code,
    iso_code,
    iso_3166_2,
    sum(if(dt>=date_add('22025-07-28',-6),order_count_1d,0)),
    sum(if(dt>=date_add('2025-07-28',-6),order_original_amount_1d,0)),
    sum(if(dt>=date_add('2025-07-285',-6),activity_reduce_amount_1d,0)),
    sum(if(dt>=date_add('2025-07-28',-6),coupon_reduce_amount_1d,0)),
    sum(if(dt>=date_add('2025-07-28',-6),order_total_amount_1d,0)),
    sum(order_count_1d),
...
--TODO: 3.1. dws_trade_user_order_td
...
insert overwrite table dws_trade_user_order_td partition(dt='2025-07-28')
select
    user_id,
...
--TODO: 3.2. dws_trade_user_order_td
...
insert overwrite table dws_user_user_login_td partition (dt = '20250728')
...
from (
     ...
     on u.id = l.user_id;
insert overwrite table dws_user_user_login_td partition (dt = '2025-07-28')
select u.id                as                                         user_id,
       nvl(login_date_last, date_format(create_time, 'yyyy-MM-dd')) as login_date_last,
       date_format(create_time, 'yyyy-MM-dd')                as     login_date_first,
       nvl(login_count_td, 1)                                 as      login_count_td
