...
-- TODO: 1. ads_traffic_stats_by_channel
--  11.1.1 各渠道流量统计
...
where dt>=date_add('2025-07-28',-recent_days+1)
group by recent_days,channel;
select * from ads_traffic_stats_by_channel;



--TODO: 1.1 ads_page_path
-- 11.1.2 路径分析
...
select churn.dt,
       user_churn_count,
       user_back_count
from (select '20250728' dt,
             count(*)   user_churn_count
      from dws_user_user_login_td
      where dt = '20250728'
        and login_date_last = '20250728') churn
         join
     (select '20250728' dt,
      ...
      from (select user_id,
            ...
            from dws_user_user_login_td
            where dt = '20250728'
              and login_date_last = '20250728') t1
               join
           (select user_id,
                   login_date_last login_date_previous
            from dws_user_user_login_td
            where dt = '20250728') t2
           on t1.user_id = t2.user_id) back
     on churn.dt = back.dt;
select * from ads_user_change;



-- TODO: 2.2. ads_user_change
--  11.2.2 用户留存率
...
union
select '2025-07-28' dt,
       login_date_first create_date,
       datediff('2025-07-28', login_date_first) retention_day,
       sum(if(login_date_last = '20250728', 1, 0)) retention_count,
       count(*) new_user_count,
       cast(sum(if(login_date_last = '20250728', 1, 0)) / count(*) * 100 as decimal(16, 2)) retention_rate
from (
         select user_id,
                login_date_last,
                login_date_first
         from dws_user_user_login_td
         where dt = '2025-07-28'
           and login_date_first >= date_add('2025-07-28', -7)
           and login_date_first < '2025-07-28'
     ) t1
group by login_date_first;
select * from ads_user_retention;
-- todo ads_user_retention创建完毕
select * from dws_user_user_login_td;



-- TODO: 2.3. ads_user_retention
--  11.2.3 用户新增活跃统计
...
select '2025-07-28' dt,
       recent_days,
       sum(if(login_date_first >= date_add('2025-07-28', -recent_days + 1), 1, 0)) new_user_count,
       count(*) active_user_count
from dws_user_user_login_td lateral view explode(array(1, 7, 30)) tmp as recent_days
where dt = '2025-07-28'
  and login_date_last >= 20250728
group by recent_days;
select * from ads_user_stats;
-- todo ads_user_stats创建完毕
select * from dws_user_user_login_td;



--TODO: 2.4. ads_user_action
-- 11.2.4 用户行为漏斗分析
...
--TODO: 2.5. ads_new_order_user_stats
-- 11.2.5 新增下单用户统计
...
where dt='2025-07-28'
  and order_date_first>=date_add('2025-07-28',-recent_days+1)
group by recent_days;
select * from ads_new_order_user_stats;
...
--TODO: 2.6. ads_order_continuously_user_count
...
from
    (
        select
            user_id,
            datediff(lead(dt,2,'2025-07-28') over(partition by user_id order by dt),dt) diff
        from dws_trade_user_order_1d
        where dt>=date_add('2025-07-28',-6)
    )t1
where diff=2;
select * from ads_order_continuously_user_count;
...
--TODO: 4.1. ads_order_to_pay_interval_avg
-- 11.4.1 下单到支付时间间隔平均值
...
where dt in ('9999-12-31','2025-07-28')
  and payment_date_id='2025-07-28';
select * from ads_order_to_pay_interval_avg;



--TODO: 4.2. ads_order_by_province
-- 11.4.2 各省份交易统计
